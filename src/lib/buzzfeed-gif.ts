...
var byteToBitArr = function (bite: number): boolean[] {
  var a: boolean[] = [];
  for (var i = 7; i >= 0; i--) {
    a.push(!!(bite & (1 << i)));
  }
...
var parseGIF = function (st: Stream, handler: any) {
  handler || (handler = {});

  // LZW (GIF-specific)
  function parseCT(entries: number) {
...
type SuperGifCallback = (error: any, data: SuperGifCallbackData | null) => void;
interface SuperGifCallbackData {
  frames: {
    data: ImageData;
    delay: number;
  }[];
...
var SuperGif = function (gif: HTMLImageElement) {
  var options = {
    //viewport position
    vp_l: 0,
    vp_t: 0,
  ...
  var stream: Stream | undefined = undefined;
  var hdr;
  ...
  var doDecodeProgress = function () {
    const pos = stream?.pos ?? 0;
    const length = stream?.data.length ?? 0;
    const progress = length ? pos / length : 0;
  };
  ...
  var load_callback: SuperGifCallback = () => {};

  var load_setup = function (callback: SuperGifCallback) {
  ...
  return {
    ...
    // },
    load(callback: SuperGifCallback) {
      const src = gif.src;
