...
export default class OnionSkinFilter {
  ...
  applyContrast(imageData: ImageData): HTMLCanvasElement {
    ...
    if (!canvas) {
      ...
      const contrastLevel = this.contrastLevel * 255;
  ...
  apply(imageData: ImageData): ImageData {
    ...
    if (!newData) {
      const { frames, opacity, unionFrames, prevColor, nextColor } = this;
      ...
      for (let i = unionFrames * -1; i < unionFrames + 1; i += 1) {
        if (i === 0) i = 1;
